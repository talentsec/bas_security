...
import { GetScenarioDetail, DeleteScenarioVersion, PublishScenarioVersion } from "@/api/scenarios";
...
function VectorDetail() {
  ...
  const [curScenario, setCurScenario] = useState<null | ResponseType.GetScenarioDetailContent>(null);
  ...
  const columns: ColumnsType<ResponseType.GetScenarioDetailContent> = [
    {
      title: "版本号",
      dataIndex: "version",
      key: "version"
  ...
  const { data: tableData, refetch } = useQuery(
    ...
    {
      select: data => {
        const list: ResponseType.GetScenarioDetailContent[] =
  ...
  const { mutate: deleteMutate } = useMutation(DeleteScenarioVersion, {
    onSuccess: data => {
      toggleDeleteModal();
  ...
  const { mutate: publishMutate } = useMutation((arg: number) => PublishScenarioVersion(arg, "PUBLISH"), {
    onSuccess: data => {
      togglePublishModal();
      if (data.code === RequestStateEnum.SUCCESS) {
  ...
  const { mutate: revorkMutate } = useMutation((arg: number) => PublishScenarioVersion(arg, "CANCEL"), {
    onSuccess: data => {
      toggleRevorkModal();
      if (data.code === RequestStateEnum.SUCCESS) {
  ...
  const handleEdit = (editType: EditType, scenario: ResponseType.GetScenarioDetailContent) => {
    ...
    switch (editType) {
      case "publish":
        ...
        break;
      case "edit":
        Navigater(`/app/my/scenario/edit?v=${scenario.id}`);
        break;
      case "revork":
        toggleRevorkModal();
        break;
  ...
  const openDetail = (item: ResponseType.GetScenarioDetailContent) => {
    Navigater(`/app/my/scenario/edit?v=${item.id}`);
  };
  ...
  const handleVersionEdit = (versionId?: number) => {
    toggleCreateModal();
    Navigater(`/app/my/scenario/edit?${id ? "id=" + id : ""}${versionId ? "&v=" + versionId : ""}`);
  };
  ...
  return (
    <div className="w-full h-full flex flex-col p-4">
      ...
      <DeleteModal
        ...
        handleOk={() => deleteMutate(Number(curScenario?.id))}
        handleCancel={toggleDeleteModal}
      ></DeleteModal>
      <RevorkModal
        open={revorkModalVisible}
        handleOk={() => revorkMutate(Number(curScenario?.id))}
        handleCancel={toggleRevorkModal}
      ></RevorkModal>
      <PublishModal
        open={publishModalVisible}
        handleOk={() => publishMutate(Number(curScenario?.id))}
