...
import { GetScenariosList, UpdateScenario, DeleteScenario } from "@/api/scenarios";
...
const MyScene = () => {
  ...
  const [curScenario, setCurScenario] = useState<null | ResponseType.GetScenariosListContent>(null);
            ...
            {user && user.id === record.createdBy ? (
              ...
              <Popover
                content={
                  <div
                    className="flex w-full justify-between cursor-pointer rounded items-center p-2 hover:bg-gray-100"
                    onClick={() => Navigater(`/app/all/scenario/${record.id}?name=${record.name}`)}
                  >
  ...
  const { data: tableData, refetch } = useQuery(
    ...
    () => GetScenariosList({ limit, page, keyword, status: filterStatus }),
    {
      select: data => {
  ...
  const handleEdit = (editType: EditType, scenarios: ResponseType.GetScenariosListContent) => {
    switch (editType) {
      case "rename":
        toggleRenameModal();
        break;
      case "detail":
        Navigater(`/app/my/vector/${scenarios.id}?name=${scenarios.name}`);
        break;
      case "delete":
        toggleDeleteModal();
        break;
