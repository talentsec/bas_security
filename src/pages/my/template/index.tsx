...
import { GetMyTemplateList, UpdateTemplate, DeleteTemplate } from "@/api/taskTemplate";
import { GetMyScenariosList } from "@/api/scenarios";
import { RequestStateEnum } from "@/type/api";
import { ReactComponent as AddIcon } from "@icon/template.svg";
...
const Index = () => {
  ...
  const [curTemplate, setCurTemplate] = useState<null | TableDateType>(null);
  ...
  const { data: tableData, refetch } = useQuery(
    ...
    () => GetMyTemplateList({ limit, page, keyword, status: filterStatus }),
    {
      select: data => {
  ...
  const handleEdit = (editType: EditType, template: TableDateType) => {
    switch (editType) {
      case "rename":
        toggleRenameModal();
        break;
  ...
  const creatVector = () => {
    Navigater("/app/my/template/edit");
  };
